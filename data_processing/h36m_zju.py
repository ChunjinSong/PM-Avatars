...
def process_h36m_zju_data(data_path, subject='S1', training_view=[0,1,2], split='train', res=None,
    ...
    # post-process mask so we can get some samples near boundary!
    """
    print('Post-processing masks')
    for i, (img, mask, sampling_mask, cam_idx) in enumerate(zip(imgs, masks, sampling_masks, cam_idxs)):
        processed_mask, processed_sampling_mask = post_process_mask(img, bkgds[cam_idx], mask, sampling_mask)
        masks[i] = processed_mask.copy()
        sampling_masks[i] = processed_sampling_mask
    """
...
if __name__ == '__main__':
    ...
    write_to_h5py(os.path.join(data_path, f"{subject}_{split}_bnd.h5"), data)
