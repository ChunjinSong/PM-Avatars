...
def get_camera_motion_bubble(c2w: np.ndarray, z_t: float = 0.1, x_deg: float = 15., y_deg: float = 25.,
                             n_step: int = 30):
...
class BaseRenderDataset(Dataset):
    ...
    def init_meta(self):
        ...
        HW = img_shape[1:3]
        self.HW = HW
        # self.idxs = np.arange(img_shape[0])
    ...
    def __getitem__(self, idx):
        ...
        bone, root_loc, kp_idx,  kp3d, skts = self.get_pose_data(idx)
    ...
    def get_pose_data(self, idx):
        ...
        skts = self.skts[kp_idx]

        return bone, root_loc, kp_idx, kp3d, skts
...
def render(config):
    ...
    raychunk = config.get('raychunk', 1024 * 5)
    ...
    if forward_type == 'render':
        ...
        gt_img_path = os.path.join(output_path, 'gt')
    ...
    for i, data in enumerate(tqdm(render_dataloader)):
        ...
        img_name = data.pop('img_name')[0]

        if forward_type in ['render', 'normal']:
            preds = model(data, forward_type=forward_type, raychunk=raychunk)
            imgs = preds['rgb_imgs']

            for j, img in enumerate(imgs):
                write_path = os.path.join(img_path, f'{img_cnt:05d}.png')
                # write_path = os.path.join(img_path, img_name)
                img = (img.cpu().numpy() * 255).astype(np.uint8)
                imageio.imwrite(write_path, img)

                # if gt_img is not None:
                #     write_path = os.path.join(gt_img_path, img_name)
            ...
            for i in range(0, N_samples, chunk):
                ...
                geom_inputs = {
                    ...
                    'skts': data['skts'],
                    'bones': data['bones'],
                    'root_locs': data['root_locs'],
                    'pts': chunk_pts,
                }
                preds = model(geom_inputs, forward_type=forward_type, raychunk=raychunk)
                densities = preds['density'].cpu().numpy()
                densities = np.maximum(densities, 0)
                density[i:i+chunk] = densities

            density = density.reshape(res, res, res, 1)
